...
@push('css_or_js')
    ...
    <style>
        ...
        .foods-search-wrapper .input-group .form-control {
            border-radius: 999px 0 0 999px;
        }
        .foods-search-wrapper .btn.btn--secondary {
...
@section('content')
    <div class="content container-fluid foods-page-modern">
        <!-- Page Header -->
        <div class="foods-header-card d-flex flex-wrap justify-content-between align-items-center mb-3">
            <div>
                <h1 class="foods-header-title mb-1">
                    قائمة الأطعمة
                </h1>
                <p class="foods-header-subtitle mb-0">إدارة أطباق مطعمك، تعديل حالتها وأسعارها بسهولة في واجهة واحدة.</p>
            </div>
            <div class="d-flex flex-wrap align-items-center mt-3 mt-md-0" style="gap: .5rem;">
                <div class="foods-header-badge mr-0 mr-md-3">
                    <i class="tio-filter-list ml-1"></i>
                    <span>إجمالي الأطعمة:</span>
                    <span id="itemCount">{{$foods->total()}}</span>
                </div>
                <div class="row g-2 align-items-center justify-content-end flex-nowrap">

                    <div class="col-auto">
                        <a href="{{route('vendor.food.stockOutList')}}" class="btn max-sm-12 btn--danger w-100"> الأطباق النافدة من المخزون</a>
                    </div>
                    <div class="col-auto">
                        <a href="{{route('vendor.food.add-new')}}" class="btn max-sm-12 btn--primary w-100"><i
                                    class="tio-add-circle"></i> إضافة طبق جديد</a>
                    </div>
        ...
        <!-- End Page Header -->
        <!-- Card -->
        <div class="card foods-list-card">
            <!-- Header -->
            <div class="card-header border-0">
                <div class="search--button-wrapper search--button-area foods-search-wrapper justify-content-end">
                    <form id="search-form">
                        @csrf
                        <!-- Search -->
                        <div class="input-group input--group">
                            <input id="datatableSearch" type="search" name="search" class="form-control foods-search-input" placeholder="مثال: بحث باسم الطبق" aria-label="بحث في قائمة الأطعمة">
                    ...
                    <div class="hs-unfold initial-89">
                        <select name="category_id" id="category" data-placeholder="اختر التصنيف"
                                class="js-data-example-ajax form-control set-filter" id="category_id"
                                data-url="{{url()->full()}}" data-filter="category_id">
                            @if($category)
                                <option value="{{$category->id}}" selected>{{$category->name}} ({{$category->position == 0?'رئيسي':'فرعي'}})</option>
                            @else
                    ...
                    <!-- Veg/NonVeg filter -->
                    <div class="col-auto">
                        <select name="category_id" data-url="{{url()->full()}}" data-filter="type" data-placeholder="الكل" class="form-control w-100 max-sm-12 set-filter">
                    ...
                    <div class="hs-unfold">
                        <a class="js-hs-unfold-invoker btn btn-white" href="javascript:;"
                            ...
                            <i class="tio-table mr-1"></i> الأعمدة
                        </a>
            ...
            <div class="table-responsive datatable-custom">
                <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table foods-table"
                    ...
                    <tr>
                        <th>م</th>
                        <th class="w-20p">اسم الطبق</th>
                        <th class="w-20p">التصنيف</th>
                        <th class="text-center">السعر</th>
                        <th class="text-center">مُوصى به</th>
                        ...
                        <tr>
                                ...
                                <a class="media align-items-center" href="{{route('vendor.food.view',[$food['id']])}}">
                                    ...
                                    <div class="media-body">
                                        <h5 class="text-hover-primary mb-0">{{Str::limit($food['name'],20,'...')}}
                                            ...
                                            <span class="badge badge-soft-warning badge-pill font-medium">نفد من المخزون</span><span class="input-label-secondary" data-toggle="tooltip" data-placement="right" data-original-title="{{translate('messages.Your_main_stock_is_out_of_stock.')}}"><img src="{{dynamicAsset('public/assets/admin/img/info-circle.svg')}}" alt="public/img"></span>
                                            @else
                            ...
                            <td>
                                <div class="btn--container justify-content-center">
                                    @if($stock_out)
                                    <a class="btn btn-sm btn--primary btn-outline-primary action-btn " href="#update-stock{{ $food['id'] }}" title="تحديث المخزون" data-toggle="modal">
                                        <i class="tio-autorenew"></i>
                                    </a>
                                    @endif
                                    <a class="btn action-btn btn--primary btn-outline-primary"
                                        href="{{route('vendor.food.edit',[$food['id']])}}" title="تعديل الطبق"><i class="tio-edit"></i>
                                    </a>
                                    <a class="btn action-btn btn--danger btn-outline-danger form-alert" href="javascript:"
                                        data-id="food-{{$food['id']}}" data-message="هل تريد حذف هذا الطبق؟" title="حذف الطبق"><i class="tio-delete-outlined"></i>
                                    </a>
                ...
                @if(count($foods) === 0)
                <div class="empty--data foods-empty text-center">
                    <img src="{{dynamicAsset('/public/assets/admin/img/empty.png')}}" alt="public">
                    <h5>
                        لا توجد أطعمة حتى الآن
                    </h5>
                </div>
                @endif
                <div class="page-area">
